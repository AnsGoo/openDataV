...
import { reactive, ref, computed, onMounted, onErrorCaptured, watch, defineComponent } from 'vue'
...
const Shape = defineComponent({
  ...
  setup(props) {
    ...
    const contentHtml = () => {
      return (<span class="error-info" v-show="isError">{{ errorInfo }}</span>
      <icon-park
        class="rotation"
      ...
      <div
        ...
        v-show="isActive"
        style={{ top: '0%', left: '0%' }}
        onMouseDown="handleStretchedShape('lt', $event)"
      ></div>
      <div
        class=['shape-point', 't', rotateClassName]
        v-show="isActive"
        style={{ top: '0%', left: '50%' }}
        onMouseDown="handleStretchedShape('t', $event)"
      ></div>
      <div
        class=['shape-point', 'rt', rotateClassName]
        v-show="isActive"
        style={{ top: '0%', left: '100%' }}
        onMouseDown="handleStretchedShape('rt', $event)"
      ></div>
      <div
        class=['shape-point', 'r', rotateClassName]
        v-show="isActive"
        style={{ top: '50%', left: '100%' }}
        onMouseDown="handleStretchedShape('r', $event)"
      ></div>
      <div
        class=['shape-point', 'rb', rotateClassName]
        v-show="isActive"
        style={{ top: '100%', left: '100%' }}
        onMouseDown="handleStretchedShape('rb', $event)"
      ></div>
      <div
        class=['shape-point', 'b', rotateClassName]
        v-show={isActive}
        style={{ top: '100%', left: '50%' }}
        onMouseDown={handleStretchedShape('b', $event)}
      ></div>
      <div
        class={shape-point}
        v-show={isActive}
        ...
        onMouseDown={handleStretchedShape('lb', $event)}
      ></div>
      <div
        :class="['shape-point', 'l', rotateClassName]"
        v-show={isActive}
        style={{ top: '50%', left: '0%' }}
        onMouseDown={handleStretchedShape('l', $event)}
      ></div>
      <slot></slot>)
    }
  }
  ...
  return () => (
    {
      ...
      onDblclick={dbselectCurComponent}
