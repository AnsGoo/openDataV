...
</code></pre><h3><a id="_356"></a>注意事项</h3><p>需要注意的事，只有在编辑模式下才需要监听组件属性变化，在预览模式下不需要监听组件属性变化，因此可以在组件中判断编辑器模式(<code>什么是编辑器模式，详见编辑器模式</code>)，来决定是不是要对属性进行监听依次可以提升组件性能</p><h2><a id="_360"></a>数据</h2><p>组件配置项对象在有一个<code>DataMode</code> 数据接入模式的属性，他定义了组件可以从那里接入数据</p><p>接入模式分为三类：</p><blockquote><ul><li><code>SELF</code>: 组件自己内部自行接入数据</li><li><code>UNIVERSAL</code>: 组件采用通用的方式接入数据</li><li><code>GLOBAL</code>: 组件从订阅全局数据</li></ul></blockquote><h3><a id="SELF_370"></a>SELF</h3><p>组件自己在内部通过不管通过<code>HTTP</code>或者<code>WebSocket</code> 自己处理数据的请求和响应,这时候组件的<code>属性栏</code>没有<code>数据</code>配置项</p><pre><code class="language-typescript">
